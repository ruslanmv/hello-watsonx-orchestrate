#!/usr/bin/env bash
# ────────────────────────────────────────────────────────────────────────────
#  watsonx Orchestrate Developer Edition Startup Script
# ────────────────────────────────────────────────────────────────────────────
set -e  # Exit on any error

# Colours ---------------------------------------------------------------------
RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; BLUE='\033[0;34m'; NC='\033[0m'

print()          { echo -e "${BLUE}[INFO]${NC} $1";   }
print_ok()       { echo -e "${GREEN}[SUCCESS]${NC} $1"; }
print_warn()     { echo -e "${YELLOW}[WARNING]${NC} $1"; }
print_err()      { echo -e "${RED}[ERROR]${NC} $1";   }

# Configuration ---------------------------------------------------------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ENV_FILE="$SCRIPT_DIR/.env"
CLEANUP_SCRIPT="$SCRIPT_DIR/cleanup_specific_resources.sh"
DEMO_DIR="$SCRIPT_DIR/demo_agent"

print "Starting watsonx Orchestrate Developer Edition…"
print "Script directory: $SCRIPT_DIR"

# Pre-flight checks -----------------------------------------------------------
print "Running pre-flight checks…"

[[ -f "$ENV_FILE" ]] || { print_err ".env file not found"; exit 1; }

docker info   >/dev/null 2>&1 || { print_err "Docker is not running"; exit 1; }
docker compose version 2>/dev/null | grep -q 'v2\.' || { print_err "Docker Compose v2 is required"; exit 1; }

if [[ -z "$VIRTUAL_ENV" ]]; then
  print_warn "No Python venv detected"; read -p "Continue anyway? (y/N): " -n 1 -r; echo
  [[ $REPLY =~ ^[Yy]$ ]] || exit 1
fi

command -v orchestrate >/dev/null 2>&1 || {
  print_err "'orchestrate' command not found – install with: pip install ibm-watsonx-orchestrate"
  exit 1
}

print_ok "Pre-flight checks passed"

# Load .env -------------------------------------------------------------------
set -a; source "$ENV_FILE"; set +a
required_vars=(WO_DEVELOPER_EDITION_SOURCE WO_ENTITLEMENT_KEY WO_INSTANCE WO_API_KEY WATSONX_APIKEY WATSONX_SPACE_ID)
for v in "${required_vars[@]}"; do [[ -z "${!v:-}" ]] && { missing="yes"; print_err "Missing env var: $v"; }; done
[[ $missing == "yes" ]] && exit 1
[[ "$WO_INSTANCE" == *"YOUR_INSTANCE_ID"* ]] && { print_err "Replace YOUR_INSTANCE_ID in WO_INSTANCE"; exit 1; }
print_ok "Environment validated"

# Stop any existing services --------------------------------------------------
print "Stopping any existing watsonx Orchestrate services…"
orchestrate server stop 2>/dev/null || true

# Demo Agent Creation Function ------------------------------------------------
create_demo_agent() {
    print "Creating demo agent and tool…"
    
    # Create demo directory
    mkdir -p "$DEMO_DIR"
    
    # Create demo tool
    cat > "$DEMO_DIR/greeting_tool.py" << 'EOF'
from ibm_watsonx_orchestrate.agent_builder.tools import tool

@tool
def greeting() -> str:
    """
    A simple greeting tool that returns a hello world message.
    
    :returns: A greeting message
    """
    return "Hello World! Welcome to watsonx Orchestrate Developer Edition!"

@tool  
def get_current_time() -> str:
    """
    Get the current date and time.
    
    :returns: Current date and time as a string
    """
    from datetime import datetime
    return f"Current time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
EOF

    # Create demo agent configuration
    cat > "$DEMO_DIR/demo_agent.yaml" << 'EOF'
spec_version: v1
kind: native
name: demo_agent
description: A simple demo agent that can greet users and provide the current time. Perfect for testing watsonx Orchestrate functionality.
instructions: |
  You are a helpful demo agent for watsonx Orchestrate Developer Edition.
  
  Your capabilities:
  - Greet users with a friendly hello world message using the greeting tool
  - Provide the current date and time using the get_current_time tool
  - Help users understand how watsonx Orchestrate works
  
  Always be friendly and helpful. When users ask for a greeting, use the greeting tool.
  When they ask for the time, use the get_current_time tool.
llm: watsonx/ibm/granite-3-8b-instruct
style: default
collaborators: []
tools: 
  - greeting
  - get_current_time
EOF

    print_ok "Demo agent files created in $DEMO_DIR"
}

# Import Demo Agent Function --------------------------------------------------
import_demo_agent() {
    print "Importing demo agent and tools…"
    
    # Import tools first
    if orchestrate tools import -k python -f "$DEMO_DIR/greeting_tool.py"; then
        print_ok "Demo tools imported successfully"
    else
        print_err "Failed to import demo tools"
        return 1
    fi
    
    # Import agent
    if orchestrate agents import -f "$DEMO_DIR/demo_agent.yaml"; then
        print_ok "Demo agent imported successfully"
    else
        print_err "Failed to import demo agent"
        return 1
    fi
}

# Start-server helper ---------------------------------------------------------
export COMPOSE_INTERACTIVE_NO_CLI=1 DOCKER_BUILDKIT=1

start_server() {
  local max=3 attempt=1
  while (( attempt <= max )); do
    print "Attempt $attempt of $max…"

    if (( attempt == 3 )); then
      print_warn "Running in pseudo-TTY to avoid TTY error…"
      cmd='script -q -c "orchestrate server start --env-file=$ENV_FILE" /dev/null'
      bash -c "$cmd"
    else
      orchestrate server start --env-file="$ENV_FILE"
    fi

    if [[ $? -eq 0 ]]; then
      print_ok "Server started successfully"; return 0
    fi

    print_warn "Server start attempt $attempt failed"

    if (( attempt == 1 )); then
      print "Running cleanup_specific_resources.sh before retry…"
      bash "$CLEANUP_SCRIPT" || print_warn "Cleanup script encountered an error"
    fi

    (( attempt++ ))
    if (( attempt <= max )); then
      print "Waiting 10 s before retry…"; sleep 10
      print "Resetting server before retry…"
      echo "I accept" | orchestrate server reset --env-file="$ENV_FILE" 2>/dev/null || true
      sleep 5
    fi
  done
  print_err "Failed to start server after $max attempts"; return 1
}

# Launch ----------------------------------------------------------------------
if ! start_server; then exit 1; fi

# Wait for services to be ready ----------------------------------------------
print "Waiting for services to initialise…"; sleep 30

# Activate local environment --------------------------------------------------
for i in {1..3}; do
  orchestrate env activate local && { print_ok "Local env activated"; break; }
  (( i == 3 )) && print_warn "Failed after 3 tries – activate manually" && break
  print_warn "Activation failed (attempt $i/3) – retrying in 10 s…"; sleep 10
done

# Check for existing agents and offer to create demo agent -------------------
print "Checking for existing agents…"
agent_count=$(orchestrate agents list 2>/dev/null | wc -l)

if [[ $agent_count -eq 0 ]]; then
    print_warn "No agents found in the current environment."
    echo
    read -p $'\e[34m[INFO]\e[0m Would you like to install a demo agent to test the platform? (Y/n): ' -n 1 -r; echo
    
    if [[ $REPLY =~ ^[Nn]$ ]]; then
        print "Skipping demo agent creation."
        print_warn "You will need to create an agent before you can use the chat interface."
        print "Use 'orchestrate agents create' or 'orchestrate agents import' to add agents."
    else
        create_demo_agent
        if import_demo_agent; then
            print_ok "Demo agent setup completed!"
            print "You can now test the platform with the demo_agent."
            print "Try asking: 'Hello' or 'What time is it?'"
        else
            print_err "Failed to set up demo agent. You may need to create agents manually."
        fi
    fi
else
    print_ok "Found $agent_count agent(s) in the environment"
fi

# Final messages --------------------------------------------------------------
print_ok "watsonx Orchestrate Developer Edition is ready!"
echo -e "
  ▸ UI  : http://localhost:3000/chat-lite
  ▸ API : http://localhost:4321/api/v1
  ▸ Docs : http://localhost:4321/docs
"

# Check if we have agents before offering to start chat
agent_count=$(orchestrate agents list 2>/dev/null | wc -l)
if [[ $agent_count -gt 0 ]]; then
    read -p $'\e[34m[INFO]\e[0m Start the chat interface now? (y/N): ' -n 1 -r; echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        print "Starting chat…"
        orchestrate chat start
    fi
else
    print_warn "No agents available. Create an agent first before starting the chat interface."
    print "Use: orchestrate agents create --name my_agent --kind native --description 'My agent' --llm watsonx/ibm/granite-3-8b-instruct"
fi

print_ok "Script completed."